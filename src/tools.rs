...
///
/// # Arguments
/// * `command` - The Bash command to execute (e.g., "ls -la").
/// * `timeout_secs` - Optional timeout in seconds (default: 30s).
///
/// # Returns
/// * `Ok(String)` - The command's stdout.
/// * `Err(Box<dyn std::error::Error + Sync + Send>)` - Error if the command fails or times out.
#[ollama_rs::function]
pub async fn run_bash_command(command: String, timeout_secs: Option<u64>) -> Result<String> {
